...
def render_2D(left_display, img_scale, objects, is_tracking_on):
    ...
    for obj in objects.object_list:
        if render_object(obj, is_tracking_on):
            ...
            if is_tracking_on:
                cv2.putText(left_display, f"Tracking ID {obj.id}", (int(position_image[0]) - 20, int(position_image[1]) - 30),
                            cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.5, (255, 255, 255, 255), 1)
            cv2.putText(left_display, f"Class {obj.raw_label}", (int(position_image[0] - 20), int(position_image[1] - 15)),
                        cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.5, (255, 255, 255, 255), 1)
