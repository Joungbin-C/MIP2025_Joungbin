...
class PalletFilling(BaseTask):
    ...
    def __init__(self, name: str = "bin_filling") -> None:
        ...
        self._target_position = np.array([-1, 0.55, -0.22]) / get_stage_units()
        self._bin_initial_position = np.array([0.55, 1.25, -0.52]) / get_stage_units()
        self._bin_size = np.array([1.2, 1.0, 0.15]) / get_stage_units()
        return
    ...
    def set_up_scene(self, scene: Scene) -> None:
        ...
        pallet_usd = self._assets_root_path + "/Isaac/Props/Pallet/pallet.usd"

        base_pos = np.array(self._bin_initial_position)
        pallet_height = 0.15
        num_layers = 1
        ...
        for i in range(num_layers):
            ...
            pallet_prim = scene.add(
                SingleRigidPrim(
                    ...
                    name=name,
                    position=pos,
                    orientation=euler_angles_to_quat(np.array([0, 0, angle_rad]))
                )
            )
            pallet_prim.set_mass(0.01)
            self._pallets.append(pallet_prim)

        self._packing_bin = self._pallets[-1]
